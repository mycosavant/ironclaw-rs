...
| Post-compaction read audit         | ✅       | ✅       | AGENTS.md rules appended to compaction summaries in daily log                                                                                                                     |
| Post-compaction context injection  | ✅       | ✅       | Synthetic turn with summary + identity context injected after compaction                                                                                                          |
...
| Skill routing blocks               | ✅       | ✅       | ActivationCriteria with keywords, patterns, tags, use_when (+15pts), dont_use_when (-30pts penalty)                                                                                |
| Skill path compaction              | ✅       | ✅       | `compact_path()` replaces $HOME with ~ in skill source XML blocks                                                                                                                 |
...
| Block-level streaming              | ✅       | ✅       | `CompletionChunk` types, `complete_stream()` on LlmProvider trait, NearAI native SSE, `Reasoning::respond_stream()`                                                               |
| Tool-level streaming               | ✅       | ✅       | `StatusUpdate::ToolProgress` + `SseEvent::ToolProgress` plumbed through all channels                                                                                              |
...
| URL allowlist (web_search/fetch)   | ✅       | ✅       | `HTTP_TOOL_URL_ALLOWLIST` env → `DomainAllowlist` on `HttpTool`; reuses sandbox proxy allowlist                                                                                    |
...
| Transcript file size in status     | ✅       | ✅       | `token_estimate` + `message_count` in `ThreadInfo`; uses `ContextBreakdown::analyze()`                                                                                             |
